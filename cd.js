...
CD.init = function(options) {
    CD.grantedBytes = localStorage.CD_grantedBytes|0;
    var size =  (window.cdOptions && window.cdOptions.requiredSize || 5 ) * 1024 * 1024;
    if (size > CD.grantedBytes) {
...
CD.prototype = {
     ...
     */
    then: function then (fn,data,timeout)   {
        if (!fn) return;
...
pause: function pause( ptime){
    var me = this;
    console.log('pause', ptime)
    this.then(function(done2){
        console.log('pause2', ptime)
        done2()
    },null, ptime)
    return this;
},
